...
const Table: React.FC<TableProps> = ({ data }) => {
  const column = [
    "Order Id", "Customer Name", "Email", "Amount", "Status", "Traffic Source"
  ];
  return (
    <div className="w-full p-5 rounded-[20px] bg-white mt-[30px] shadow-sm">
      <div className="flex items-center justify-between mb-5">
        <h3 className="text-[#2C2F50] text-lg font-semibold">Order Details</h3>
        ...
        <div className="w-full h-[600px] overflow-y-auto rounded-lg">
          <table className="w-full border-collapse text-sm text-left">
            <thead className="sticky top-0 bg-[#EDF1FF] text-[#2C2F50]">
              <tr>
                {column?.map((col, i) => (
                  <th className="p-3 font-semibold" key={i}>
      ...
      <div className="mt-5 flex items-center justify-between">
        <p className="text-xs font-normal text-[#8A8B9E]">
